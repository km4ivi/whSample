...
The defaults for these additional arguments are *backups=5*, *irisData=F*, *seed=NULL* and *keepOrg=T". The default seed will tell *sampler* to use the current system time in milliseconds. Any number can be used as a seed. Whichever one is used will be listed in the *Report* output tab. The keep-original option (*keepOrg*) defaults to TRUE, but should be *keepOrg=F* for populations larger than about a million records since Excel's row limit is 1,048,576 and *sampler* adds some header and blank lines to its output.
